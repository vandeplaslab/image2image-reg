...
from image2image_reg.enums import ValisCrop
...
def transform_valis(
    ...
    """Transform files using Valis."""
    from image2image_io.readers import get_simple_reader

    from image2image_reg.valis.transform import (
    ...
    # load transformation
    project = ValisReg.from_path(project)

    # transform files
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    # get attached modality
    if attached_to not in project.modalities:
        raise ValueError(f"Modality {attached_to} not found in project")
    attached_to_modality = project.modalities[attached_to]
    ...
    for file in files:
        ...
        if reader.reader_type == "image":
            ...
            transform_attached_points(
                ...
                [file],
                output_dir=output_dir,
                source_pixel_size=pixel_size or 1.0,
                crop=crop,
                suffix=suffix,
            )
        elif reader.reader_type == "shapes":
