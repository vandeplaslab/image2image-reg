...
from valis.warp_tools import calc_d, warp_xy

logger = logger.bind(src="Matcher")


# Metrics found in both the valid metrics ang kernel methods in sklearn.metrics.pairwise. Issue is that metrics are
...
# Epislon error to avoid division by 0
EPS: float = np.finfo(float).eps

# filter_method parameter in match_desc_and_kp is set to this,
# Grid-based Motion Statistics will be used to remove poor matches
GMS_NAME: str = "GMS"

# str: If filter_method parameter in match_desc_and_kp is set to this, will be used to remove poor matches
RANSAC_NAME: str = "RANSAC"


# The defulat filter_method value, either RANSAC_NAME or GMS_NAME
DEFAULT_MATCH_FILTER: str = RANSAC_NAME

# Default ransac threshold
DEFAULT_RANSAC: int = 7


def convert_distance_to_similarity(d: float, n_features: int = 64) -> float:
    """
...
def convert_similarity_to_distance(s: float, n_features: int = 64) -> float:
    ...
    """
    return -np.log(s + EPS) / (1 / n_features)  # type: ignore[no-any-return]


def filter_matches_ransac(kp1_xy: np.ndarray, kp2_xy: np.ndarray, ransac_val: int = DEFAULT_RANSAC) -> tuple:
    ...
    if kp1_xy.shape[0] >= 4:
        ...
        f"Need at least 4 keypoints for RANSAC filtering, but only have {kp1_xy.shape[0]}. {traceback_msg}"
...
def filter_matches_gms(
    kp1_xy: np.ndarray,
    kp2_xy: np.ndarray,
    feature_d: np.ndarray,
    ...
    """
    kp1 = cv2.KeyPoint_convert(kp1_xy.tolist())  # type: ignore[attr-defined]
    kp2 = cv2.KeyPoint_convert(kp2_xy.tolist())  # type: ignore[attr-defined]
    matches = [cv2.DMatch(_queryIdx=i, _trainIdx=i, _imgIdx=0, _distance=feature_d[i]) for i in range(len(kp1_xy))]
    gms_matches = cv2.xfeatures2d.matchGMS(  # type: ignore[attr-defined]
        img1_shape,
        img2_shape,
        kp1,
        kp2,
...
def match_desc_and_kp(
    ...
    match_info12 = MatchInfo(
        ...
        matched_desc1=matched_desc1,
        matches12=desc1_match_idx,
        matched_kp2_xy=matched_kp2_xy,
        matched_desc2=matched_desc2,
        matches21=desc2_match_idx,
        match_distances=match_distances,
        distance=mean_unfiltered_distance,
        similarity=mean_unfiltered_similarity,
    ...
    match_info21 = MatchInfo(
        ...
        matches12=desc2_match_idx,
        matched_kp2_xy=matched_kp1_xy,
        matched_desc2=matched_desc1,
        matches21=desc1_match_idx,
        match_distances=match_distances,
        distance=mean_unfiltered_distance,
        similarity=mean_unfiltered_similarity,
        metric_name=metric_name,
    ...
    filtered_match_info12 = MatchInfo(
        ...
        match_distances=filterd_match_distances,
        distance=mean_filtered_distance,
        similarity=mean_filtered_similarity,
        metric_name=metric_name,
        metric_type=metric_type,
    )

    filtered_match_info21 = MatchInfo(
        matched_kp1_xy=filtered_matched_kp2_xy,
        matched_desc1=filterd_matched_desc2,
        matches12=good_matches21,
        matched_kp2_xy=filtered_matched_kp1_xy,
        matched_desc2=filterd_matched_desc1,
        matches21=good_matches12,
        match_distances=filterd_match_distances,
        distance=mean_filtered_distance,
        similarity=mean_filtered_similarity,
        metric_name=metric_name,
...
class Matcher:
    ...
    def match_images(self, desc1, kp1_xy, desc2, kp2_xy, additional_filtering_kwargs=None, *args, **kwargs):
        ...
        if self.match_filter_method == GMS_NAME:
            if additional_filtering_kwargs is not None:
                ...
                filtering_kwargs.update({"scaling": self.scaling, "thresholdFactor": self.gms_threshold})
            else:
                print(
                    Warning(
        ...
        match_info12, filtered_match_info12, match_info21, filtered_match_info21 = match_desc_and_kp(
            desc1,
            kp1_xy,
            desc2,
            kp2_xy,
