...
def read_elastix_iteration_data(iteration_txt: Path | str) -> dict[str, np.ndarray]:
    """
...
def read_elastix_iteration_dir(registration_dir: Path | str) -> dict[int, dict[int, dict[str, np.ndarray]]]:
    ...
    iter_info_fps = [Path(fp) for fp in natsorted([str(fp) for fp in iter_info_fps])]
...
def read_elastix_intermediate_transform_data(transform_txt: Path | str) -> dict[str, str]:
    """
...
def read_elastix_transform_dir(registration_dir: Path | str) -> dict[int, dict[int, dict[str, str]]]:
    """
    ...
    """
    tform_info_fps = sorted(Path(registration_dir).glob("TransformParameters*"))
    tform_info_fps = [Path(fp) for fp in natsorted([str(fp) for fp in tform_info_fps])]
...
def create_iteration_plot(iteration_dict: dict[str, np.ndarray], plot_title: str) -> plt.Figure:
    """
    ...
    """
    fig, ((plt1, plt2), (plt3, plt4), (plt5, plt6)) = plt.subplots(3, 2, sharex=True, figsize=(8, 6))
