...
class SerialRigidRegistrar:
    ...
    def __init__(
        ...
        self.name = name
        self.img_file_list = get_image_files(img_dir, ordered=is_sorted)
        ...
        if self.align_to_reference is False and reference_img_f is not None:
            og_ref_name = get_image_name(reference_img_f)
            logger.warning(
                f"The reference was specified as {og_ref_name} but `align_to_reference=False`, "
                f"and so images will be aligned serially *towards* the reference image. If you would like all images"
                f" to be *directly* aligned to {og_ref_name},  then set `align_to_reference=True`. "
                f"Note that in both cases, {og_ref_name} will remain unwarped.",
            )

    def generate_img_obj_list(self, feature_detector):
        ...
        sorted_img_list = [io.imread(self.img_dir / f, True) for f in self.img_file_list]
        ...
        for i in trange(self.size, desc="Detecting features", unit="image", leave=None):
            img_f = self.img_file_list[i]
            img = sorted_img_list[i]

            img_name = get_image_name(img_f)
            img_obj = Image(img, self.img_dir / img_f, i, name=img_name)
            img_obj.padded_shape_rc = out_shape
            img_obj.T = get_padding_matrix(img.shape, img_obj.padded_shape_rc)
    ...
    def match_sorted_imgs(self, matcher_obj, keep_unfiltered=False):
        ...
        keep_unfiltered : bool
            Whether matcher_obj should store unfiltered matches
        ...
        n_cpu = get_cpu_count(1)
        # TODO replace with mpire
        pqdm(range(self.size), match_adj_img_obj, n_jobs=n_cpu, desc="Matching images", unit="image", leave=None)

        # with parallel_backend("threading", n_jobs=n_cpu):
        #     Parallel()(delayed(match_adj_img_obj)(i) for i in range(self.size))

    def match_imgs(self, matcher_obj, keep_unfiltered=False):
        ...
        # pbar = tqdm(total=n_comparisions, desc="Matching images", unit="image", leave=None)
        ...
        n_cpu = get_cpu_count(1)
        pqdm(range(self.size), match_img_obj, n_jobs=n_cpu, desc="Matching images", unit="image", leave=None)
        # with parallel_backend("threading", n_jobs=n_cpu):
    ...
    def get_neighbor_matches_idx(self, img_obj, prev_img_obj, next_img_obj):
        ...
        xy_to_prev_idx = index_2d_to_1d(xy_to_prev[:, 1], xy_to_prev[:, 0], img_obj.image.shape[1])
        xy_to_next_idx = index_2d_to_1d(xy_to_next[:, 1], xy_to_next[:, 0], img_obj.image.shape[1])
    ...
    def get_common_desc(
        self, current_img_obj: Image, neighbor_obj: Image, nf_kp_idx: np.ndarray
        ...
        current_img_obj : Image
            Current Image
        neighbor_obj : Image
            Image that is a neighbor to `current_img_obj`
        nf_kp_idx : ndarray
    ...
    def neighbor_match_filtering(self, img_obj, prev_img_obj, next_img_obj, tform, matcher_obj):
        ...
        def measure_d(src_xy: np.ndarray, dst_xy: np.ndarray, tform, M=None):
            """Measure distance between warped corresponding points."""
            if M is None:
                tform.estimate(src=dst_xy, dst=src_xy)
                M = tform.params
            warped_xy = warp_xy(src_xy, M)
            d = np.median(calc_d(warped_xy, dst_xy))
        ...
        if len(nf_prev_idx) >= 3:
            ...
            common_matches_d, common_matches_M = measure_d(common_kp, common_prev_kp, tform)

            original_d, _ = measure_d(to_prev_match_info12.matched_kp1_xy, to_prev_match_info12.matched_kp2_xy, tform)

            original_with_neighbor_filter_d, _ = measure_d(
                to_prev_match_info12.matched_kp1_xy, to_prev_match_info12.matched_kp2_xy, tform, M=common_matches_M
            )
            ...
            if common_matches_d < original_d and original_with_neighbor_filter_d <= original_d:
                ...
                updated_prev_match_info12, _, updated_prev_match_info21, _ = matcher_obj.match_images(
                    desc1=filtered_desc, kp1_xy=filtered_kp, desc2=filtered_prev_desc, kp2_xy=filtered_prev_kp
                )

                updated_next_match_info12, _, updated_next_match_info21, _ = matcher_obj.match_images(
                    desc1=_filtered_desc, kp1_xy=_filtered_kp, desc2=filtered_next_desc, kp2_xy=filtered_next_kp
                )
    ...
    def update_match_dicts_with_neighbor_filter(self, tform, matcher_obj):
        ...
        for i, img_obj in enumerate(self.img_obj_list):
            ...
            improved, updated_prev_match_info12, updated_next_match_info12 = self.neighbor_match_filtering(
                img_obj, prev_img_obj, next_img_obj, tform, matcher_obj
            )
        ...
        # Update matches
        for i, img_obj in enumerate(self.img_obj_list):
            if img_obj.name not in new_matches:
                continue
            prev_idx = i - 1
            prev_img_obj = self.img_obj_list[prev_idx]
    ...
    def sort(self):
        ...
        """
        sorted_D, sorted_idx, optimal_Z = order_distance_matrix(self.unsorted_distance_mat)
    ...
    def get_iter_order(self):
        ...
        if self.reference_img_f is not None:
            ref_img_name = get_image_name(self.reference_img_f)
        else:
            ref_img_name = None

        obj_names = [img_obj.name for img_obj in self.img_obj_list]
        ref_img_idx = get_ref_img_idx(obj_names, ref_img_name)
        self.reference_img_idx = ref_img_idx
        self.reference_img_f = self.img_obj_list[ref_img_idx].full_img_f
        self.iter_order = get_alignment_indices(self.size, ref_img_idx)
    ...
    def align_to_prev_check_reflections(self, transformer, feature_detector, matcher_obj, keep_unfiltered=False):
        """Use key points to align image to previous image in the stack, checking if reflection improves alignment.
        ...
        for moving_idx, fixed_idx in tqdm(self.iter_order, desc="Finding transforms", unit="image", leave=None):
            img_obj = self.img_obj_list[moving_idx]
            prev_img_obj = self.img_obj_list[fixed_idx]

            if fixed_idx == self.reference_img_idx:
                prev_matrix = ref_img_obj.T.copy()
            ...
            unreflected_warped_src_xy = warp_xy(to_prev_match_info.matched_kp1_xy, transformer.params)
            _, unreflected_d = measure_error(
                to_prev_match_info.matched_kp2_xy, unreflected_warped_src_xy, prev_img_obj.image.shape
            )
            ...
            dst_xy = warp_xy(prev_img_obj.kp_pos_xy, prev_matrix)
            for rx in [False, True]:
                for ry in [False, True]:
                    if not rx and not ry:
                        continue

                    reflect_matrix = get_reflection_M(rx, ry, img_obj.image.shape)
                    reflected_img = warp_img(
                        img_obj.image, reflect_matrix @ img_obj.T, out_shape_rc=img_obj.padded_shape_rc
                    )
                    ...
                    reflected_warped_src_xy = warp_xy(filtered_match_info12.matched_kp1_xy, transformer.params)
                    _, reflected_d = measure_error(
                        filtered_match_info12.matched_kp2_xy, reflected_warped_src_xy, prev_img_obj.padded_shape_rc
                    )
                    reflected_d_vals.append(reflected_d)
                    reflection_M.append(reflect_matrix)
                    transforms.append(transformer.params)

                    # Move matched features to position in original images
                    img_inv_M = np.linalg.inv(reflect_matrix @ img_obj.T)
                    prev_img_inv_M = np.linalg.inv(prev_matrix)

                    filtered_match_info12.matched_kp1_xy = warp_xy(filtered_match_info12.matched_kp1_xy, img_inv_M)
                    filtered_match_info12.matched_kp2_xy = warp_xy(filtered_match_info12.matched_kp2_xy, prev_img_inv_M)

                    filtered_match_info21.matched_kp1_xy = warp_xy(filtered_match_info21.matched_kp1_xy, prev_img_inv_M)
                    filtered_match_info21.matched_kp2_xy = warp_xy(filtered_match_info21.matched_kp2_xy, img_inv_M)

                    reflected_matches12.append(filtered_match_info12)
                    reflected_matches21.append(filtered_match_info21)

                    if keep_unfiltered:
                        unfiltered_match_info12.matched_kp1_xy = warp_xy(
                            unfiltered_match_info12.matched_kp1_xy, img_inv_M
                        )
                        unfiltered_match_info12.matched_kp2_xy = warp_xy(
                            unfiltered_match_info12.matched_kp2_xy, prev_img_inv_M
                        )

                        unfiltered_match_info21.matched_kp1_xy = warp_xy(
                            unfiltered_match_info21.matched_kp1_xy, prev_img_inv_M
                        )
                        unfiltered_match_info21.matched_kp2_xy = warp_xy(
            ...
            best_reflect_matrix = reflection_M[best_idx]
            best_matrix = transforms[best_idx]
            img_obj.to_prev_A = best_matrix
            img_obj.reflection_M = best_reflect_matrix
            prev_matrix = img_obj.reflection_M @ img_obj.T @ img_obj.to_prev_A

            ref_x, ref_y = best_reflect_matrix[[0, 1], [0, 1]] < 0
            if ref_x or ref_y:
    ...
    def align_to_prev(self, transformer):
        ...
        for moving_idx, fixed_idx in tqdm(self.iter_order, desc="Finding transforms", unit="image", leave=None):
            ...
            to_prev_match_info = img_obj.match_dict[prev_img_obj]
            src_xy = warp_xy(to_prev_match_info.matched_kp1_xy, img_obj.T)
            dst_xy = warp_xy(to_prev_match_info.matched_kp2_xy, prev_M)
    ...
    def optimize(self, affine_optimizer):
        ...
        """
        ref_img_obj = self.img_obj_list[self.reference_img_idx]
        ref_warped = warp_img(ref_img_obj.image, matrix=ref_img_obj.T, out_shape_rc=ref_img_obj.padded_shape_rc)

        for moving_idx, fixed_idx in tqdm(self.iter_order, desc="Optimizing transforms", unit="image", leave=None):
            img_obj = self.img_obj_list[moving_idx]
            prev_img_obj = self.img_obj_list[fixed_idx]
            ...
            warped_img = warp_img(img_obj.image, matrix=M, out_shape_rc=img_obj.padded_shape_rc)

            to_prev_match_info = img_obj.match_dict[prev_img_obj]
            before_src_xy = warp_xy(to_prev_match_info.matched_kp1_xy, M)
            before_dst_xy = warp_xy(to_prev_match_info.matched_kp2_xy, prev_M)
            before_tre, before_med_d = measure_error(before_src_xy, before_dst_xy, warped_img.shape)

            # Get mask
            img_mask = np.ones(img_obj.image.shape[0:2], dtype=np.uint8)
            warped_img_mask = warp_img(img_mask, matrix=M, out_shape_rc=img_obj.padded_shape_rc)

            prev_img_mask = np.ones(prev_img_obj.image.shape[0:2], dtype=np.uint8)
            warped_prev_img_mask = warp_img(prev_img_mask, matrix=prev_M, out_shape_rc=prev_img_obj.padded_shape_rc)
            ...
            with hide_stdout():
                _, optimal_M, _ = affine_optimizer.align(
                    moving=warped_img, fixed=prev_img, mask=mask, initial_M=None, moving_xy=moving_xy, fixed_xy=fixed_xy
                )

            # Keep optimal M if it actually improved alignment
            initial_cst = affine_optimizer.cost_fxn(warped_img, prev_img, mask)

            after_src_xy = warp_xy(to_prev_match_info.matched_kp1_xy, M @ optimal_M)
            after_dst_xy = warp_xy(to_prev_match_info.matched_kp2_xy, prev_M)

            optimal_reg_img = warp_img(warped_img, matrix=optimal_M, out_shape_rc=img_obj.padded_shape_rc)

            after_cst = affine_optimizer.cost_fxn(optimal_reg_img, prev_img, mask)

            after_tre, after_med_d = measure_error(after_src_xy, after_dst_xy, warped_img.shape)
    ...
    def calc_warped_img_size(self):
        ...
        for i in range(self.size):
            ...
            img_corners_rc = get_corners_of_image(img_obj.image.shape)
            warped_corners_xy = warp_xy(img_corners_rc[:, ::-1], M)
    ...
    def finalize(self):
        ...
        for i in trange(self.size, desc="Finalizing", unit="image", leave=None):
            img_obj = self.img_obj_list[i]
        ...
        for i, img_obj in enumerate(self.img_obj_list):
            ...
            img_obj.registered_img = warp_img(img=img_obj.image, matrix=img_obj.M, out_shape_rc=(h, w))
    ...
    def wiggle_to_ref(self, transformer):
        ...
        for img_obj in self.img_obj_list:
            ...
            rigid_reg_moving_xy = warp_xy(matches.matched_kp1_xy, matrix=img_obj.M)
            rigid_reg_fixed_xy = warp_xy(matches.matched_kp2_xy, matrix=img_obj.fixed_obj.M)
    ...
    def summarize(self):
        ...
        for i in range(0, self.size):
            ...
            og_tre_list[i], og_med_d_list[i] = measure_error(temp_current_pts, temp_prev_pts, img_obj.image.shape)

            current_pts = warp_xy(temp_current_pts, img_obj.M)
            prev_pts = warp_xy(temp_prev_pts, prev_img_obj.M)

            tre_list[i], med_d_list[i] = measure_error(current_pts, prev_pts, img_obj.image.shape)

            similarities = convert_distance_to_similarity(
                current_to_prev_matches.match_distances, current_to_prev_matches.matched_desc1.shape[0]
            )

            _, weighted_med_d_list[i] = measure_error(current_pts, prev_pts, img_obj.image.shape, similarities)
        ...
        summary_df["series_d"] = calc_total_error(summary_df.D.values[non_ref_idx])
        summary_df["series_tre"] = calc_total_error(summary_df.TRE.values[non_ref_idx])
        summary_df["series_weighted_d"] = calc_total_error(summary_df.D_weighted.values[non_ref_idx])
...
def register_images(
    ...
    with MeasureTimer() as timer:
        ...
        # print("\n======== Detecting features\n")
        registrar.generate_img_obj_list(feature_detector)
        ...
        if valis_obj is not None:
            if valis_obj.create_masks:
                ...
                for img_obj in registrar.img_obj_dict.values():
                    ...
                    features_in_mask_idx = get_xy_inside_mask(xy=img_obj.kp_pos_xy, mask=slide_obj.rigid_reg_mask)
        ...
        if affine_optimizer is not None:
            ...
            registrar.optimize(affine_optimizer)
        registrar.finalize()
        ...
        if dst_dir is not None:
            registered_img_dir = dst_dir / "registered_images"
            registered_img_dir.mkdir(exist_ok=True, parents=True)
            registered_data_dir = dst_dir / "data"
            registered_data_dir.mkdir(exist_ok=True, parents=True)

            # print("\n======== Summarizing alignments\n")
            summary_df = registrar.summarize()
            summary_file = registered_data_dir / (name + "_results.csv")
            summary_df.to_csv(summary_file, index=False)

            registrar.summary = summary_df

            # print("\n======== Saving results\n")
            pickle_file = registered_data_dir / (name + "_registrar.pickle")
            pickle.dump(registrar, open(pickle_file, "wb"))

            n_digits = len(str(registrar.size))
            for img_obj in registrar.img_obj_list:
                f_out = "".join([str.zfill(str(img_obj.stack_idx), n_digits), "_", img_obj.name, ".png"])

                io.imsave(registered_img_dir / f_out, img_obj.registered_img.astype(np.uint8))
        registrar.clear_unused_matches()
    print(f"\n======== Rigid registration complete in {timer()}\n")
    return registrar
