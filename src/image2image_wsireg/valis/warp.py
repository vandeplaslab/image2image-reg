...
class ArrayWrapper:
    """Wrapper for numpy array that has a couple of extra functions."""
    ...
    def resize(self, scale: float) -> np.ndarray:
        """Resize image."""
        new_size = np.round(np.array(self.shape) * scale).astype(int)
        return cv2.resize(self, dsize=new_size)

    def affine(
        self, matrix: list | np.ndarray, out_shape: tuple[int, int], interp_method: str = "linear"
    ) -> np.ndarray:
        """Affine transform.

        Affine matrix should have already been inverted. This is because scipy does a backward warp, while
        opencv does a forward warp. This means that the matrix should be inverted before being passed to
        `affine_transform` in scipy.
        """
        from scipy.ndimage import affine_transform

        if matrix.ndim == 1 and len(matrix) == 4:
            matrix = scale_matrix_to_affine(matrix)

        kws = {"order": 0, "mode": "constant"}
        if interp_method == "bicubic":
            kws["order"] = 3

        return affine_transform(self.image, matrix, output_shape=out_shape, **kws)

    def bandjoin(self, other: np.ndarray) -> np.ndarray:
        """Bandjoin."""
        return np.dstack([self.image, other])

    def mapim(self, **kwargs):
        """"""
        ...

    def extract_area(self, **kwargs):
        """Extract area."""
        ...
    ...
    @classmethod
    def xyz(cls, width: int, height: int) -> np.ndarray:
        """Return an array of x, y coordinates."""
        x, y = np.meshgrid(np.arange(width), np.arange(height))
        return np.dstack([x, y])


def get_ref_img_idx(img_f_list: list[str], ref_img_name: str | None = None) -> int:
    ...
    if ref_img_name is None:
        ...
        ref_img_name = get_image_name(os.path.split(ref_img_name)[1])
        img_names = [get_image_name(f).lower() for f in img_f_list]
...
def get_alignment_indices(n_imgs: int, ref_img_idx: int | None = None) -> list[ImageShape]:
...
def remove_invasive_displacements(bk_dxdy, M, src_shape_rc, out_shape_rc, inpaint_holes=False):
    ...
    if M is not None:
        affine_mask = warp_img(
            np.full(src_shape_rc, 255, dtype=np.uint8), M, out_shape_rc=out_shape_rc, interp_method="nearest"
        )
...
def rescale_img(img: np.ndarray, scaling: float) -> np.ndarray:
    """Rescale image."""
    return ArrayWrapper(img).resize(scaling)


def scale_matrix_to_affine(matrix: list | np.ndarray) -> np.ndarray:
    """Scale matrix to affine matrix."""
    matrix = np.asarray(matrix)
    if matrix.ndim == 2 and matrix.shape[0] == (3, 3):
        return matrix
    if matrix.ndim == 1 and len(matrix) == 4:
        sx, xx, xy, sy = matrix
        return np.array([sx, xx, 0, xy, sy, 0, 0, 0, 1]).reshape(3, 3)
    raise ValueError("Matrix must be 1d array with 4 elements or 2d array with shape (3, 3)")


def resize_img(img: np.ndarray, out_shape_rc: ImageShape, interp_method: str = "linear") -> np.ndarray:
    """Resize image."""
    # TODO: this is broken
    wrap = ArrayWrapper(img)
    out_h, out_w = out_shape_rc

    src_shape_rc = np.array([wrap.height, wrap.width])
    sy, sx = np.array(out_shape_rc) / src_shape_rc
    matrix = scale_matrix_to_affine(np.array([sx, 0, 0, sy]))
    return wrap.affine(matrix, out_shape=(out_w, out_h), interp_method=interp_method)


def scale_dxdy(dxdy: np.ndarray, out_shape_rc: ImageShape):
    wrap = ArrayWrapper(np.dstack(dxdy))

    sxy = (np.array(out_shape_rc) / np.array([wrap.height, wrap.width]))[::-1]
    scaled_dx = float(sxy[0]) * wrap[0]
    scaled_dy = float(sxy[1]) * wrap[1]
...
def get_src_img_shape_and_M(
    matrix: np.ndarray,
    transformation_src_shape_rc: ImageShape,
    transformation_dst_shape_rc: ImageShape,
    dst_shape_rc: ImageShape,
) -> tuple[ImageShape, np.ndarray]:
    ...
    be warped to land in the image with shape dst_shape_rc.
    ...
    scaled_matrix = scale_matrix(matrix, *dst_sxy)

    scaled_unwarped_corners = warp_xy(scaled_warped_corners, matrix=np.linalg.inv(scaled_matrix))
    src_slide_bbox = xy2bbox(scaled_unwarped_corners)
    src_shape_rc = np.round(src_slide_bbox[2:] + src_slide_bbox[:2]).astype(int)

    return src_shape_rc, scaled_matrix


def save_img(dst_f: PathLike, img: np.ndarray, thumbnail_size: ImageShape | None = None) -> None:
    ...
    if thumbnail_size is not None:
        vips_wh = np.array([wrap.width, wrap.height])
        s = np.min(thumbnail_size / vips_wh)
        if s < 1:
            out_img = wrap.resize(s)
            wrap = ArrayWrapper(out_img)
    wrap.to_file(dst_f)
...
def get_pts_in_bbox(xy, xywh):
    ...
    in_bbox_idx = np.where((xy[:, 0] >= x0) & (xy[:, 0] < x1) & (xy[:, 1] >= y0) & (xy[:, 1] < y1) is True)[0]
...
def get_shape(img: np.ndarray) -> np.ndarray:
    ...
    """
    wrap = ArrayWrapper(img)
    shape_rc = np.array([wrap.height, wrap.width])
    if img.ndim > 2:
        ndim = img.shape[2]
    else:
        ndim = 1
...
def get_grid_bboxes(shape_rc, bbox_w, bbox_h, inclusive=False):
    ...
    bbox_list = [
        [tl_x[i, j], tl_y[i, j], tl_x[i + 1, j + 1] - tl_x[i, j], tl_y[i + 1, j + 1] - tl_y[i, j]]
...
#     for i in range(nrow):
#         col_tiles = [tile_list[index_2d_to_1d(i, j, ncol=ncol)] for j in col_range]
#         row_mosaic = col_tiles[0]
#
#         for j in range(1, ncol):
#             tile_idx = index_2d_to_1d(i, j, ncol)
#
...
#         bottom_idx = index_2d_to_1d(i, 0, ncol)
...
def index_2d_to_1d(row: int, col: int, ncol: int) -> int:
    """Convert 2d index to 1d."""
    index = (ncol * row) + col
    return index


def index_1d_to_2d(index: int, ncol: int):
...
def get_triangular_mesh(x_pos, y_pos):
    ...
    for bbox_xy in grid_boxes_xy:
        ...
        left_face = [vert_dict[bbox_tuples[0]], vert_dict[bbox_center_xy], vert_dict[bbox_tuples[3]]]

        top_face = [vert_dict[bbox_tuples[0]], vert_dict[bbox_tuples[1]], vert_dict[bbox_center_xy]]

        right_face = [vert_dict[bbox_center_xy], vert_dict[bbox_tuples[1]], vert_dict[bbox_tuples[2]]]

        btm_face = [vert_dict[bbox_center_xy], vert_dict[bbox_tuples[2]], vert_dict[bbox_tuples[3]]]
...
def get_corners_of_image(shape_rc: ImageShape) -> np.ndarray:
    """
...
def _vips2numpy_pre_22(vi):
    ...
    img = np.ndarray(
        buffer=vi.write_to_memory(), dtype=format_to_dtype[vi.format], shape=[vi.height, vi.width, vi.bands]
    )
    if vi.bands == 1:
        img = img[..., 0]
...
def pad_img(img: np.ndarray, padded_shape: ImageShape | None = None) -> tuple[np.ndarray, np.ndarray]:
    """Pad image."""
...
def warp_img(
    ...
    bk_dxdy: np.ndarray | None = None,
    out_shape_rc: ImageShape | None = None,
    transformation_src_shape_rc: ImageShape | None = None,
    ...
    if transformation_dst_shape_rc is None:
        if bk_dxdy is not None:
            ...
            warped_transformation_src_corners_xy = warp_xy(transformation_src_corners_rc[:, ::-1], matrix)
    ...
    # Taking ceiling can prevent shifting down & right by 1 pixel when warping images much larger than the one used
    # to find M
    ...
    if bk_dxdy is not None:
        apply_non_rigid = True
    else:
        apply_non_rigid = False

    if not any([apply_rigid, apply_non_rigid, apply_crop]):
        return wrap.image
    ...
    if apply_rigid:
        if not np.all(src_sxy == 1):
            ...
            rigid_transform = transform.ProjectiveTransform()
            rigid_transform.estimate(warped_corners, img_corners_xy)
            warped_matrix = rigid_transform.params

        else:
            warped_matrix = matrix

        warped_matrix = np.linalg.inv(warped_matrix)

        affine_warped = ArrayWrapper(
            wrap.affine(
                warped_matrix,
                out_shape=(out_shape_rc[1], out_shape_rc[0]),
    ...
    if apply_non_rigid:
        # Scale dxdy map
        wrap_dxdy = ArrayWrapper(np.dstack(bk_dxdy))
        # if not isinstance(bk_dxdy, pyvips.Image):
        #     temp_dxdy = numpy2vips(np.dstack(bk_dxdy))
        # else:
        #     temp_dxdy = bk_dxdy

        vips_dxdy: ArrayWrapper
        if dst_sxy is not None:
            scaled_dx = float(dst_sxy[0]) * wrap_dxdy[0]
            scaled_dy = float(dst_sxy[1]) * wrap_dxdy[1]
        ...
        index = ArrayWrapper.xyz(affine_warped.width, affine_warped.height)
...
def warp_img_inv(
    ...
    if do_non_rigid:
        ...
        warp_dxdy = vips_dxdy.affine(
            S, oarea=[0, 0, img.width, img.height], interpolate=interpolator, premultiplied=True
        )
        ...
        try:
            # Option to set backround color in mapim added in libvips 8.13
            nr_warped = img.mapim(
                warp_index, premultiplied=True, background=bg_color, extend=bg_extender, interpolate=interpolator
            )
...
def get_padding_matrix(img_shape_rc: ImageShape, out_shape_rc: ImageShape) -> np.ndarray:
    """Get transformation matrix to pad an image, so that it fits in a larger image."""
...
def get_reflection_M(reflect_x: bool, reflect_y: bool, shape_rc: ImageShape) -> np.ndarray:
...
def get_img_area(img_shape_rc: ImageShape, M: np.ndarray = None) -> float:
    """Get image area."""
...
def calc_d(pt1: np.ndarray, pt2: np.ndarray) -> np.ndarray:
    """
    ...
    -------
    d : [N]
        distance between corresponding points in pt1 and pt2
    """
...
def smooth_dxdy(dxdy, grid_spacing_ratio=0.015, sigma_ratio=0.005, method="gauss"):
    ...
    if method.lower().startswith("c"):
        ...
        SmoothBivariateSpline(subgrid_r_flat, subgrid_c_flat, sub_dy.reshape(-1))
...
def _warp_pt_vips(
    ...
    nonrigid_xy = warp_xy_non_rigid(
        xy=rigid_xy_in_tile, dxdy=[region_dxdy[..., 0], region_dxdy[..., 1]], displacement_shape_rc=[bbox_h, bbox_w]
    )
...
def warp_xy(
    ...
    matrix: np.ndarray | None = None,
    transformation_src_shape_rc: ImageShape | None = None,
    transformation_dst_shape_rc: ImageShape | None = None,
    src_shape_rc: ImageShape | None = None,
    dst_shape_rc: ImageShape | None = None,
    bk_dxdy: np.ndarray | None = None,
    fwd_dxdy: np.ndarray | None = None,
):
    """
    Warp xy points using M and/or bk_dxdy/fwd_dxdy. If bk_dxdy is provided, it will be inverted to  create fwd_dxdy.
    ...
    if matrix is None and not do_non_rigid:
        return xy
...
def warp_xy_inv(
    ...
    if matrix is not None:
        xy_inv = warp_xy(xy_in_rigid, matrix=np.linalg.inv(matrix))
    else:
        xy_inv = xy_in_rigid
...
def get_inside_mask_idx(xy, mask):
    ...
    inside_mask = np.array([cv2.pointPolygonTest(mask_cnt[0], tuple(xy[i]), False) for i in range(xy.shape[0])])
...
def mask2xy(mask):
    ...
    bbox = np.array([[min_x, min_y], [max_x + 1, min_y], [max_x + 1, max_y + 1], [min_x, max_y + 1]])

    return bbox


def bbox2mask(x: int, y: int, w: int, h: int, shape: ImageShape) -> np.ndarray:
...
def xy2bbox(xy: np.ndarray) -> np.ndarray:
    """Get bounding box from xy coordinates."""
    min_x = np.min(xy[:, 0])
    max_x = np.max(xy[:, 0])
    min_y = np.min(xy[:, 1])
    max_y = np.max(xy[:, 1])
...
def bbox2xy(xywh: np.ndarray) -> np.ndarray:
    """
...
def calc_total_error(error: np.ndarray) -> float:
    """
...
def measure_error(
    src_xy: np.ndarray, dst_xy: np.ndarray, shape: ImageShape, feature_similarity: ty.Any = None
) -> tuple[float, float]:
    """
...
def scale_matrix(matrix: np.ndarray, scale_x: float, scale_y: float) -> np.ndarray:
...
def get_overlapping_poly(mesh_poly_coords):
    ...
    pqdm(range(n_poly), clip_poly, n_jobs=n_cpu, unit="image", leave=None)
...
#     logger.warning("Looking for folds", None, rgb=Fore.YELLOW)
...
#         logger.warning("Removing folds using regularizaation", None, rgb=Fore.YELLOW)
