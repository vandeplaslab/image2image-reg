...
class Transform:
    ...
    def __init__(self, elastix_transform: dict):
        ...
        self.output_origin = [float(p) for p in self.elastix_transform.get("Origin")]
        self.output_direction = [float(p) for p in self.elastix_transform.get("Direction")]
        self.resample_interpolator = self.elastix_transform.get("ResampleInterpolator")[0]
    ...
    def compute_inverse_nonlinear(self) -> None:
        ...
        tform_to_disp_field = sitk.TransformToDisplacementFieldFilter()
        tform_to_disp_field.SetOutputSpacing(self.output_spacing)
        tform_to_disp_field.SetOutputOrigin(self.output_origin)
        tform_to_disp_field.SetOutputDirection(self.output_direction)
        tform_to_disp_field.SetSize(self.output_size)

        displacement_field = tform_to_disp_field.Execute(self.itk_transform)
    ...
    def as_array(
        ...
        if self.is_linear:
            ...
            full_matrix[0:2, n_dim - 1] = -np.dot(tmatrix, center) + translation + center

            return full_matrix
        warn("Non-linear transformations can not be represented converted" "to homogenous matrix")
        return None
